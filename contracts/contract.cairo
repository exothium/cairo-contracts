...
func set_signer{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (vehicle_owner_address_var) = vehicle_owner_address.read(vehicle_id=vehicle_id)
    assert_not_zero(vehicle_owner_address_var)
    let (caller_address) = get_caller_address()
    assert (vehicle_owner_address_var) = caller_address
    assert_not_zero(signer_address)

    # Initialize the vehicle's signer
    vehicle_signer_address.write(vehicle_id=vehicle_id, value=signer_address)
    return ()
end
